...
kmt2a_domains <- read_csv("kmt2a_domain_variant_counts.csv", show_col_types = FALSE)

# ✅ Load gnomAD variant data (from the modified script)
gnomad_for_plot <- read_csv("kmt2a_variants.csv", show_col_types = FALSE)
...
plot_gnomad <- ggplot() +
  ...
  ggtitle("gnomAD Missense Variants in kmt2a") +
  custom_theme

# ✅ Save the figure
ggsave("lollipop_gnomAD_kmt2a.png", plot_gnomad, width = 10, height = 4, dpi = 300)
