...
# Load gene list (all lowercase for matching)
gene_list <- tolower(read_lines("gene_symbols.csv"))

# Load each day's expression file
...
day3 <- day3 %>% filter(tolower(gene_name) %in% gene_list)
day6 <- day6 %>% filter(tolower(gene_name) %in% gene_list)
day12 <- day12 %>% filter(tolower(gene_name) %in% gene_list)
...
write.csv(merged, "whole_genome_isoform_expression_levels_with_npc.csv", row.names = FALSE)
...
# Define function for ANOVA + Tukey
...
# Save all and significant
write.csv(anova_results, "whole_genome_isoform_statistical_results_with_npc.csv", row.names = FALSE)

significant <- anova_results %>% filter(!is.na(best_day), fdr < 0.05, fold_change > 2)
write.csv(significant, "whole_genome_isoform_significant_with_npc_fdr_fc.csv", row.names = FALSE)

cat("✅ Statistical analysis done. Top hits:\n")
print(head(significant %>% arrange(fdr), 10))
