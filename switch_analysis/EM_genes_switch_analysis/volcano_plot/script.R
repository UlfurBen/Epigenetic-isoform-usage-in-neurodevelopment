...
expr_data <- read.csv("em_isoform_expression_levels.csv")
...
volcano_plot <- ggplot(combined_volcano_df, aes(x = log2FC, y = -log10(fdr))) +
  ...
  labs(
    title = "EM Genes Faceted Volcano Plot of Isoform Expression Between Neurodevelopmental Stages",
    x = "log2(Fold Change)",
    y = "-log10(FDR)"
  )

# 8. Save and show plot
ggsave("em_genes_faceted_isoform_volcano_plot_labeled.png", volcano_plot, width = 14, height = 6, dpi = 300)
print(volcano_plot)
