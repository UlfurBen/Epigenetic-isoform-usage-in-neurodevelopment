...
ranked <- ranked %>%
  mutate(
    odds_ratio_capped = ifelse(is.infinite(odds_ratio), 1024, odds_ratio),
...
# 4. Plot
em_plot <- ggplot(ranked, aes(x = log2_odds_ratio, y = neg_log10_min_fdr, color = composite_score)) +
  geom_point(size = 3.5, alpha = 0.85) +
  scale_color_gradient(low = "gray80", high = "darkred") +
  labs(
    title = "Composite scoring of EM genes",
    subtitle = "log2(odds ratio), -log10(min FDR), composite score coloring",
    x = "log2(Odds Ratio)",
...
# 5. Save plot
ggsave("em_genes_composite_plot.pdf", plot = em_plot, width = 10, height = 6, dpi = 300)
